...
namespace lve {
    Monitor::Monitor(GLFWmonitor* monitorin) : monitor(monitorin), monitorWidth(0), monitorHeight(0), physicalWidth(0), physicalHeight(0), scaleX(0.0f), scaleY(0.0f), xPos(0), yPos(0) {
        if (!monitor) {
    ...
    std::string Monitor::formatMode() const {
        return FORMAT("({}x{}, Bits rgb{}{}{}, RR:{}Hz)",
